...
public:
    ...
    {   
        ...
        auto dist = latest_distance.value()->ranges[17];
        if(dist < 0.5)
        {     
            std::cout <<"DIST: " << dist <<"\n";
...
public:
          ...
          node_(rclcpp::Node::make_shared("stop_node"))
    {
        ...
        publisher3_ = node_->create_publisher<std_msgs::msg::String>("/web_message", 10);
    }
    // It is mandatory to define this STATIC method.
    static PortsList providedPorts()
    {
            ...
            <RetryUntilSuccessful num_attempts="5000">
                <Sequence>
                    <RetryUntilSuccessful num_attempts="200">
                    	<Fallback name="battery_check_fallback">
                	    <BatteryOK/>
                	    <SaySomething message="Battery low: Please recharge"/>
            		</Fallback>
                        <Fallback name="turn">
                            <CheckFront/>
                            <Sequence>
