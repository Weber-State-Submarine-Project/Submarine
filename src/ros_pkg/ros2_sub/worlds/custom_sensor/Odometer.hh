...
namespace custom
{
  ...
  class Odometer : public gz::sensors::Sensor
  {
    ...
    public: void SetVelocity(const gz::math::Vector3d &_velocity);

    /// \brief Get the latest velocity of the robot.
    /// \return The latest velocity set to the odometer.
    public: const gz::math::Vector3d &Velocity() const;

    /// \brief Noise that will be applied to the sensor data
    private: gz::sensors::NoisePtr noise{nullptr};

    private: gz::math::Vector3d velocity;
